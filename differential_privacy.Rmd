...
A researcher does not have direct access to the database, instead they have at thier disposal a query function
...
where $E$ stands for *expected error*, and expectations average over all possible training and testing data sets for the learning algorithm.
...
$$ \omega \prop \sum_{i = 1}^{n} cov(\hat\mu_i, y_i) $$
...
If we allow a modeling algorithm to access data in a differentialy private manner, then we should expect good generalization properties.
...
This can be exaggerated if we are interested in a non-parameteric model
...
Using the Laplace mechanism renders these queries differentialy private
...
Dwork, Feldman, Hardt, Pitassi, Reingold, and Roth describe a differentialy private way to query a hold out that prevents over fitting to the result.
...
If the perfomance evaluated on the training set is different from the performace on the hold out

$$\| E_{t}(\phi) - E_{h}(\phi) \| > T + \nu$$ 

then output a preterbed version of the hold out performance
...
According to Dwork, et. al. this mechanism allows the modeler to avoid over fitting to their hold out in a differentialy private manner for a surprisingly long time.
