...
In 2008 Arvind Narayanan and Vitaly Shmatikov sucessfully deanonymized the Netflix competition data by attacking with the IMDB database as an external source.

Harvard reseacrcher Latanya Sweeney attacked the  Massachusetts Group Insurance Commission medical database by linking it with voter registration records, allowing her to identify the medical history of the Governor.
...
A refinement using Stein's lemma makes this even more apparant (under certain assumptions)
...
$$ f(s) = \sum_{j \mid x_j = s} 1 + Laplase(0, \theta)$$
$$ g(s) = \sum_{j \mid x_j = s} y_j + Laplase(0, \theta)$$
...
Often a scientist will use a *hold out* dataset during the construction of a model to 
...
Unfortuantely, these uses are in direct conflict.

The more decisions that are made with the assistence of a hold out, the more biased its estiamte of the model's expected performance becomes.


## Example: A Re-usable holdout

Dwork, Feldman, Hardt, Pitassi, Reingold, and Roth describe a differntially private way to query a hold out that pervents overfitting to the result.
...
The modeler requests the pperformance of the learner $\phi$.
...
According to Dwork, et. al. this mechanism allows the modeler to avoid overfitting to thier hold out in a differentiably private manner for a suprisingly long time.

Moritz Hardt and Arvim Blum describe a modification of the algorithm above that applies to Machine Learning competition leaderboards.
...
This dicipline is still in it's infancy, so there is certainly more to come!  Keep following!